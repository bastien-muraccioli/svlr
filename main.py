...
def main():
    ...
    parser.add_argument(
        "--server", type=str, default="127.0.0.1", help="Robot server address"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="HuggingFace",
        help="LLM provider: HuggingFace or OpenAI",
    )
    parser.add_argument(
        "--llm_temperature",
        type=float,
    ...
    # Image
    parser.add_argument(
        "--show_image", action="store_true", help="Show the captured image"
    )
    parser.add_argument(
        "--save_image", action="store_true", help="Save the captured image"
    )
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        ...
        while True:
            ...
            if args.camera_topic != "":
                image = get_camera_image_ros(topic=args.camera_topic)
                print("Image from ROS")
            else:
