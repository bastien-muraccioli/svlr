...
class ControlLoop:
    def __init__(self, args: argparse.Namespace, node=None):
        ...
        self.prompt_generator = PromptGenerator(
            robot_info=self.robot_info, perception=self.perception
        )
    ...
    def perception_run(self, image):
        environment_description_list, vlm_raw_output, frame_with_masks_and_centers = (
            self.perception.run(image)
        )
        return (
            environment_description_list,
            vlm_raw_output,
            frame_with_masks_and_centers,
        )
    ...
    def action_counter(self):
        return (
            self.action.robot_action_class.action_step,
            self.action.robot_action_class.current_action_step,
        )
